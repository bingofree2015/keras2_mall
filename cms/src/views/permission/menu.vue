<template>
	<div class="page-container">
		...
		<el-row>
			...
			<el-col :span="14" class="top-bar">
				<el-form :inline="true" :size="miniSize">
					<el-form-item>
						<ext-button :label="$t('action.add')" @click="handleAdd" icon="el-icon-ali-add" perms="sys:menu:add" type="primary" />
					</el-form-item>
					<el-form-item>
						<el-button-group>
		...
		<!--表格树内容栏-->
		<el-table :data="menuTreeData" :size="miniSize" element-loading-text="$t('action.loading')" stripe style="width: 100%;" v-loading="loading">
			<el-table-column label="ID" minWidth="60" prop="id"></el-table-column>
			<table-tree-column @sendTreeData="getTreeData" label="名称" minWidth="180" prop="name" treeKey="id"></table-tree-column>
			...
			<el-table-column :label="$t('action.operation')" fixed="right" minWidth="200">
				<template slot-scope="scope">
					<ext-button :label="$t('action.edit')" @click="handleEdit(scope.row)" icon="el-icon-ali-bianji" perms="sys:menu:edit" />
					<ext-button :label="$t('action.delete')" @click="handleDelete(scope.row)" icon="el-icon-ali-shanchu" perms="sys:menu:delete" type="danger" />
				</template>
			</el-table-column>
		</el-table>
		...
		<el-dialog :close-on-click-modal="false" :title="!formData.id ? '新增' : '修改'" :visible.sync="dialogVisible" width="40%">
			<el-form :model="formData" :rules="dataRule" :size="normalSize" @keyup.enter.native="submitForm()" label-width="80px" ref="formData">
				...
				<el-form-item label="上级菜单" prop="parentName">
					<popup-tree-input :currentChangeHandle="handleTreeSelectChange" :data="popupTreeData" :nodeKey="''+formData.parentId" :prop="formData.parentName===null||formData.parentName===''?'顶级菜单':formData.parentName" :props="popupTreeProps"></popup-tree-input>
				</el-form-item>
				<el-form-item label="授权标识" prop="perms" v-if="formData.type !== 0">
					<el-input placeholder="如: sys:user:add, sys:user:edit, sys:user:delete" v-model="formData.perms"></el-input>
				</el-form-item>
				...
				<el-form-item label="排序编号" prop="orderNum" v-if="formData.type !== 2">
					<el-input-number :min="0" controls-position="right" label="排序编号" v-model="formData.orderNum"></el-input-number>
				</el-form-item>
				<el-form-item label="菜单图标" prop="icon" v-if="formData.type !== 2">
					<el-row>
						<el-col :span="22">
							<el-input :readonly="false" class="icon-list__input" placeholder="菜单图标名称（如：el-icon-ali-shouye）" v-model="formData.icon" v-popover:iconListPopover></el-input>
						</el-col>
						<el-col :span="2" class="icon-list__tips">
							<falcon-tooltip />
						</el-col>
