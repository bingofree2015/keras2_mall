<template>
	<el-dialog :close-on-click-modal="false" :modal="false" @open="openDialog" title="订单详情" v-bind="$attrs" v-on="$listeners" width="60%">
		<el-tabs v-model="activeName">
				...
				<el-form :size="miniSize" class="dialog-container" label-width="100px">
					<div class="group-container">
						...
						<el-row>
							...
							<el-col :span="8">
								<el-form-item label="支付状态:">{{mapAlias.order.pay_state[viewData.payState]}}</el-form-item>
							</el-col>
						</el-row>
						<el-row>
							<el-col :span="8">
								<el-form-item label="发货状态:">{{mapAlias.order.ship_state[viewData.shipState]}}</el-form-item>
							</el-col>
							<el-col :span="8">
						...
						<el-row>
							<el-col :span="8">
								<el-form-item label="支付方式:">{{mapAlias.payment_type[viewData.paymentCode]}}</el-form-item>
							</el-col>
							<el-col :span="8">
			...
			<el-tab-pane label="支付单/退款单" name="payRefundItem">
				...
				<el-table :data="viewData.billPayments" :size="miniSize" stripe style="width: 100%">
					...
					<el-table-column :formatter="(row, column)=>{return env.columnFormatter(row[column.property], 'payment_type')}" label="支付方式" min-width="80" prop="paymentCode"></el-table-column>
					...
					<el-table-column :formatter="(row, column)=>{return env.columnFormatter(row[column.property], 'bill_payment', column.property)}" align="center" label="支付状态" min-width="80" prop="state"></el-table-column>
				...
				<el-table :data="viewData.billRefunds" :size="miniSize" stripe style="width: 100%">
					...
					<el-table-column :formatter="(row, column)=>{return env.columnFormatter(row[column.property], 'payment_type')}" label="退款方式" min-width="80" prop="paymentCode"></el-table-column>
					...
					<el-table-column :formatter="(row, column)=>{return env.columnFormatter(row[column.property], 'bill_refund', column.property)}" label="退款状态" min-width="80" prop="state"></el-table-column>
			...
			<el-tab-pane label="订单记录" name="orderRecord">
				<el-table :data="viewData.orderLogs" :size="miniSize" stripe style="width: 100%">
					...
					<el-table-column :formatter="(row, column)=>{ return env.columnFormatter(row[column.property], 'order_log', column.property)}" label="操作类型" min-width="80" prop="type"></el-table-column>
					<el-table-column label="描述" min-width="280" prop="msg"></el-table-column>
					<el-table-column :formatter="(row,column)=>{return env.formatDateTime(row[column.property])}" label="时间" min-width="110" prop="createdAt"></el-table-column>
				</el-table>
			</el-tab-pane>
			<el-tab-pane label="订单备注" name="orderRemark">
				<el-form :model="formData" :rules="formDataRules" :size="normalSize" label-width="80px" ref="formData">
					<el-form-item label="备注内容" prop="mark">
						<el-input :autosize="{ minRows: 8, maxRows: 16}" :rows="8" placeholder="请输入内容" type="textarea" v-model="formData.mark"></el-input>
					</el-form-item>
				</el-form>
				<div class="dialog-footer">
					<el-button :size="miniSize" @click.native="$emit('update:visible', false)" round>{{$t('action.cancel')}}</el-button>
					<el-button :loading="loading" :size="miniSize" @click.native="submitForm" round type="primary">{{$t('action.submit')}}</el-button>
				</div>
			</el-tab-pane>
		</el-tabs>
	</el-dialog>
