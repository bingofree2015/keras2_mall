<template>
	<div class="page-container">
		<!--新增编辑界面-->
		<el-form :model="formData" :rules="formDataRules" :size="normalSize" class="edit-dialog-container" label-width="100px" ref="formData">
			<el-row>
				<el-col :span="24">
			...
			<el-row>
				<el-col :span="16">
						...
						<el-col :span="10">
							<el-form-item label="物流公司:" prop="logiCode">
								<el-select placeholder="请选择" v-model="formData.logiCode">
									<el-option :key="logi.logiCode" :label="logi.logiName" :value="logi.logiCode" v-for="logi in logistics"></el-option>
								</el-select>
							</el-form-item>
						</el-col>
					</el-row>
					...
					<el-form-item label="重量设置:">
						<el-col :span="8">
							...
							<el-select placeholder="请选择" v-model="formData.firstUnit">
								<el-option :key="index" :label="val" :value="key" v-for="(val, key, index) in mapAlias.ship_unit"></el-option>
							</el-select>
						</el-col>
						<el-col :span="8">
							续重
					...
					<el-form-item label="配送费用:">
						<el-col :span="6">
							首重
							<el-input-number :max="10" :min="1" controls-position="right" label="请输入首重配送费用" v-model="formData.firstunitPrice"></el-input-number>
						</el-col>
						<el-col :span="6">
							续重
							<el-input-number :max="10" :min="1" controls-position="right" label="请输入续重配送费用" v-model="formData.continueUnitPrice"></el-input-number>
						</el-col>
						<el-col :span="6">
							<el-form-item label="排序:" prop="sort">
								<el-input-number :max="100" :min="0" controls-position="right" label="请输入顺序码" v-model="formData.sort"></el-input-number>
							</el-form-item>
						</el-col>
					</el-form-item>

					<el-form-item label="商品满多少" prop="goodsMoney">
						<el-input-number :max="99" :min="0" controls-position="right" label="请输入数量" v-model="formData.goodsMoney"></el-input-number>
					...
					<el-table :data="formData.areaFee" style="width: 100%" v-if="formData.type === 2">
						...
						<el-table-column label="选择" minWidth="130" prop="areas">
							<template slot-scope="scope">
								<pick-area :areaIds="scope.row.areas.map(v=>v.id)" @chosedAreas="(areaIds)=>{return chosedAreas(areaIds,scope.row)}"></pick-area>
							</template>
						</el-table-column>
						<el-table-column label="首重费用" minWidth="130" prop="firstUnitAreaPrice">
							<template slot-scope="scope">
								<el-input :size="miniSize" placeholder="请输入首重费用" v-model="scope.row.firstUnitAreaPrice"></el-input>
							</template>
						</el-table-column>
						<el-table-column label="续重费用" minWidth="130" prop="continueUnitAreaPrice">
							<template slot-scope="scope">
								<el-input :size="miniSize" placeholder="请输入续重费用" v-model="scope.row.continueUnitAreaPrice"></el-input>
							</template>
						</el-table-column>
						<el-table-column align="center" fixed="right" label="操作" minWidth="80">
							<template slot="header">
								<el-button @click.prevent="handleAddAreaFee()" round size="mini" style="float:right" type="primary">添加</el-button>
							</template>
							<template slot-scope="scope">
								<el-button @click="handleDeleteAreaFee(scope.row.idx)" circle icon="el-icon-delete" size="mini" type="danger"></el-button>
							</template>
						</el-table-column>
					</el-table>
				</el-col>
