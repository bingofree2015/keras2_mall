<template>
	<!--个人中心界面-->
	<el-dialog :close-on-click-modal="false" :modal="false" class="edit-dialog-container" title="个人中心" v-bind="$attrs" v-on="$listeners" width="40%">
		<el-form :model="formData" :rules="formDataRules" :size="size" label-width="80px" ref="formData">
			<el-row>
				<el-col :span="14" class="top-bar">
					...
					<el-form-item label="角色" prop="roles">{{formData.roles?formData.roles.map(v=>v.remark).toString():''}}</el-form-item>
				</el-col>
				<el-col :span="10">
		...
		<div class="dialog-footer" slot="footer">
			<el-button :size="size" @click.native="$emit('update:visible', false)" round>{{$t('action.cancel')}}</el-button>
			<el-button :loading="editLoading" :size="size" @click.native="submitForm" round type="primary">{{$t('action.submit')}}</el-button>
		</div>
	</el-dialog>
</template>
