	...
	<div class="page-container">
		...
		<el-row justify="space-between" type="flex">
			...
			<el-col :span="14" class="top-bar">
				<el-form :inline="true" :model="filters" :size="normalSize">
					<el-form-item>
						<el-input placeholder="请输入内容" v-model="filters.value">
							...
							<ext-button :label="$t('action.search')" @click="queryForPaginatedList()" icon="el-icon-ali-chazhaobiaodanliebiao" perms="permission:sys_user:view" slot="append" type="primary" />
						</el-input>
					</el-form-item>
					<el-form-item>
						<ext-button :label="$t('action.add')" @click="handleAdd" icon="el-icon-ali-add" perms="permission:sys_user:add" type="primary" />
					</el-form-item>
					<el-form-item>
						<el-button-group>
							...
							<el-tooltip content="列显示" placement="top">
								<el-button @click="displayFilterColumnsDialog" icon="el-icon-ali-Filter" round></el-button>
							</el-tooltip>
				...
				<!--表格显示列界面-->
				<table-column-filter-dialog :columns="columns" @handleFilterColumns="handleFilterColumns" ref="tableColumnFilterDialog"></table-column-filter-dialog>
			</el-col>
		</el-row>

		<!--表格内容栏-->
		<ext-table :batchDelete="batchDelete" :columns="columns" :count="paginated.attrs.count" :data="paginated.list" :disableIds="disableIds" :operations="operations" :operationWidth="operationWidth" :pageSize="paginated.attrs.limit" :permsBatchDelete="permsBatchDelete" @queryForPaginatedList="queryForPaginatedList"></ext-table>

		<!--新增编辑界面-->
		<el-dialog :close-on-click-modal="false" :title="isCreating?'新增':'编辑'" :visible.sync="editDialogVisible" width="40%">
			<el-form :model="formData" :rules="formDataRules" :size="normalSize" label-position="right" label-width="80px" ref="formData">
				<el-row>
					<el-col :span="14" class="top-bar">
					...
					<el-col :span="10">
						<el-form-item label="头像" prop="attachment">
							<change-image-icon :imgUrl="formData.attachment? formData.attachment.path:''" @chosedImageIcon="chosedIcon"></change-image-icon>
						</el-form-item>
					</el-col>
				</el-row>
				<el-row>
					...
					<el-col :span="24">
						<el-form-item label="角色" prop="roleIds">
							<el-select multiple placeholder="请选择" style="width:100%" v-model="formData.roleIds">
								<el-option :key="item.id" :label="item.remark" :value="item.id" v-for="item in allRoles"></el-option>
							</el-select>
						</el-form-item>
					</el-col>
				</el-row>
