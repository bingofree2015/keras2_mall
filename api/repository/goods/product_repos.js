const { Product } = require('../../models');

const logger = require('tracer').colorConsole();

class ProductRepos {
  constructor () {
    this._instance = null;
  }

  async create (product) {
    let _result = {
      succeed: 0, // 1:成功0:失败
      code: 0, // 错误码
      description: '', // 错误信息
      data: null, // 本身就是一个json字符串
    };

    try {
      Object.keys(product).forEach((field) => {
        if (!product[field] && product[field] != 0) {
          delete product[field];
        }
      });
      if (product.userId && product.title) {
        let _product = await Product.findOne({
          where: { userId: product.userId, title: product.title },
          raw: true,
        });
        if (_product) {
          _result = { succeed: 0, code: 101, description: '名称重复' };
        } else {
          _product = await Product.create(product);
          _result = {
            succeed: 1,
            code: 200,
            description: '成功',
            data: _product,
          };
        }
      } else {
        _result = {
          succeed: 0,
          code: 100,
          description: `参数错误 -> product:${JSON.stringify(product)}`,
        };
      }
    } catch (err) {
      logger.error(err);
      _result = { succeed: 0, code: 500, description: err.message || err.stack || '系统错误' };
    }

    return _result;
  }

  async delete (ids) {
    let _result = {
      succeed: 0,
      code: 0,
      description: '',
      data: null,
    };

    try {
      if (Array.isArray(ids) && ids.length > 0) {
        const _affectedCount = await Product.destroy({ where: { id: ids } });
        if (_affectedCount == 0) {
          _result = { succeed: 0, code: 102, description: '记录不存在' };
        } else {
          _result = { succeed: 1, code: 200, description: '成功' };
        }
      } else {
        _result = { succeed: 0, code: 100, description: '参数错误' };
      }
    } catch (err) {
      logger.error(err);
      _result = { succeed: 0, code: 500, description: err.message || err.stack || '系统错误' };
    }

    return _result;
  }

  async update (id, product) {
    let _result = {
      succeed: 0,
      code: 0,
      description: '',
      data: null,
    };

    try {
      Object.keys(product).forEach((field) => {
        if (!product[field] && product[field] != 0) {
          delete product[field];
        }
      });
      if (id) {
        const _userId = product.userId;
        const _title = product.title;
        if (_userId && _title) {
          let _product = await Product.findOne({
            where: { id, userId: _userId, title: _title },
            raw: true,
          });
          if (_product) {
            const _ret = await Product.update(product, { where: { id } });
            const _affectedCount = _ret[0];
            if (_affectedCount == 0) {
              _result = { succeed: 0, code: 102, description: '记录不存在' };
            } else {
              _result = {
                succeed: 1,
                code: 200,
                description: '成功',
                data: { ...product, id },
              };
            }
          } else {
            _product = await Product.findOne({
              where: { userId: _userId, title: _title, id: { $ne: id } },
              raw: true,
            });
            if (_product) {
              _result = { succeed: 0, code: 101, description: `名称 [${_title}] 重复` };
            } else {
              const _ret = await Product.update(product, { where: { id } });
              const _affectedCount = _ret[0];
              if (_affectedCount == 0) {
                _result = { succeed: 0, code: 102, description: '记录不存在' };
              } else {
                _result = {
                  succeed: 1,
                  code: 200,
                  description: '成功',
                  data: { ...product, id },
                };
              }
            }
          }
        } else {
          const _ret = await Product.update(product, { where: { id } });
          const _affectedCount = _ret[0];
          if (_affectedCount == 0) {
            _result = { succeed: 0, code: 102, description: '记录不存在' };
          } else {
            _result = {
              succeed: 1,
              code: 200,
              description: '成功',
              data: { ...product, id },
            };
          }
        }
      } else {
        _result = { succeed: 0, code: 100, description: `参数错误 -> id:${id}` };
      }
    } catch (err) {
      logger.error(err);
      _result = { succeed: 0, code: 500, description: err.message || err.stack || '系统错误' };
    }

    return _result;
  }

  async get (id) {
    let _result = {
      succeed: 0,
      code: 0,
      description: '',
      data: null,
    };

    try {
      const _product = await Product.findOne({ where: { id }, raw: true });
      if (_product) {
        _result = {
          succeed: 1,
          code: 200,
          description: '成功',
          data: _product,
        };
      } else {
        _result = { succeed: 0, code: 102, description: '数据不存在' };
      }
    } catch (err) {
      logger.error(err);
      _result = { succeed: 0, code: 500, description: err.message || err.stack || '系统错误' };
    }
    return _result;
  }

  async list (searchKey, offset, limit) {
    let _result = {
      succeed: 0,
      code: 0,
      description: '',
      data: null,
    };

    const _where = {};
    const _excludeKeys = [];
    if (searchKey) {
      Object.keys(searchKey).forEach((field) => {
        if (!searchKey[field] && searchKey[field] != 0) {
          delete searchKey[field];
        }
      });
      for (const _key in searchKey) {
        if (typeof searchKey[_key] === 'string' && !_excludeKeys.includes(_key)) {
          _where[_key] = {
            $like: `%${searchKey[_key]}%`,
          };
        } else {
          _where[_key] = searchKey[_key];
        }
      }
    }

    const _datas = [];
    try {
      if ((offset == 0 || offset) && limit) {
        const _products = await Product.findAndCountAll({
          where: _where,
          offset,
          limit,
          raw: true,
        });
        for (const _product of _products.rows) {
          _datas.push(_product);
        }
        _result = {
          succeed: 1,
          code: 200,
          description: '成功',
          data: { list: _datas, count: _products.count },
        };
      } else {
        const _products = await Product.findAll({ where: _where, raw: true });
        for (const _product of _products) {
          _datas.push(_product);
        }
        _result = {
          succeed: 1,
          code: 200,
          description: '成功',
          data: { list: _datas },
        };
      }
    } catch (err) {
      logger.error(err);
      _result = { succeed: 0, code: 500, description: err };
    }

    return _result;
  }

  // 构造一个广为人知的接口，供用户对该类进行实例化
  static getInstance () {
    if (!this._instance) {
      this._instance = new ProductRepos();
    }
    return this._instance;
  }
}

module.exports = ProductRepos.getInstance();
