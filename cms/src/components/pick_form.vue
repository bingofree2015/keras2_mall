<template>
    <div>
        <ext-button :size="miniSize" @click="handlePickForm()" icon="el-icon-ali-Newxuanzeshangpinxuanzhong" label="选择表单" type="primary" />
            ...
            <el-table :data="paginated.list" :size="miniSize" @selection-change="multipleChoiceChange" element-loading-text="$t('action.loading')" stripe v-loading="loading">
                ...
                <el-table-column align="center" label="选择" minWidth="80" v-else-if="selectionType === 0">
                    <template slot-scope="scope">
                        <el-radio @change.native="choiceChange(scope.$index,scope.row)" :label="scope.$index" v-model="radio">&nbsp;</el-radio>
                    </template>
                </el-table-column>
