<template>
	<div class="page-container">
				...
				<el-form :inline="true" :model="filters" :size="normalSize">
					<el-form-item>
						<el-input placeholder="请输入内容" v-model="filters.value">
							...
							<ext-button :label="$t('action.search')" @click="queryForPaginatedList()" icon="el-icon-ali-chazhaobiaodanliebiao" perms="permission:role:view" slot="append" type="primary" />
						</el-input>
					</el-form-item>
					<el-form-item>
						<ext-button :label="$t('action.add')" @click="handleAdd" icon="el-icon-ali-add" perms="permission:role:add" type="primary" />
					</el-form-item>
					<el-form-item>
						<el-button-group>
		...
		<ext-table
			...
			:stripe="false"
			@handleCurrentRowChange="handleRoleSelectChange"
			@queryForPaginatedList="queryForPaginatedList"
		></ext-table>
		...
		<el-container class="role-menu-container">
			<el-header style="height:38px">
				...
				<el-checkbox :disabled="(this.selectRole.id === null|| this.disableIds.includes(this.selectRole.id))" :size="miniSize" @change="handleCheckAll" v-model="checkAll">
					<b>全选</b>
				</el-checkbox>
				<ext-button :disabled="(this.selectRole.id === null|| this.disableIds.includes(this.selectRole.id))" :label="$t('action.reset')" @click="resetSelection" perms="permission:role:edit" type="primary" />
				<ext-button :disabled="(this.selectRole.id === null|| this.disableIds.includes(this.selectRole.id))" :label="$t('action.submit')" :loading="authLoading" @click="submitAuthForm" perms="permission:role:edit" type="primary" />
			</el-header>
			<el-main style="padding:0px">
				<el-tree :data="menuData" :props="defaultProps" :show-checkbox="!this.disableIds.includes(this.selectRole.id)" :size="miniSize" element-loading-text="拼命加载中" node-key="id" ref="menuTree" v-loading="menuLoading">
		...
		<!--新增编辑界面-->
		<el-dialog :close-on-click-modal="false" :title="isCreating?'新增':'编辑'" :visible.sync="editDialogVisible" width="40%">
...
.page-container ::v-deep .role-menu-container {
    margin-left: 10px;
    width: 60%;
}

.role-menu-container ::v-deep .el-header {
    text-align: right;
