...
class NoticeRepos {
  ...
  async create (notice) {
    ...
    try {
      ...
      if (notice.title) {
        let _notice = await Notice.findOne({ where: { title: notice.title }, raw: true });
        if (_notice) {
      ...
      logger.error(err);
      _result = { succeed: 0, code: 500, description: err.notice || err.stack || '系统错误' };
    }
  ...
  async update (id, notice) {
    ...
    try {
      ...
      if (id) {
        ...
        if (_title) {
          let _notice = await Notice.findOne({ where: { id, title: _title }, raw: true });
          if (_notice) {
            ...
            _notice = await Notice.findOne({ where: { title: _title, id: { $ne: id } }, raw: true });
            if (_notice) {
              _result = { succeed: 0, code: 101, description: '记录重复' };
            } else {
              const _ret = await Notice.update(notice, { where: { id } });
  ...
  async get (id) {
    ...
    try {
      const _notice = await Notice.findOne({ where: { id }, raw: true });
      if (_notice) {
        _result = {
          succeed: 1,
          code: 200,
    ...
    try {
      if ((offset == 0 || offset) && limit) {
        ...
        _result = {
          ...
          code: 200,
          description: '成功',
          data: { list: _datas, count: _notices.count },
        };
      } else {
        const _notices = await Notice.findAll({ where: _where, raw: true });
