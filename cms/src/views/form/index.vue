<template>
	<div class="page-container">
				...
				<el-form :inline="true" :model="filters" :size="normalSize">
					<el-form-item>
						<el-input placeholder="请输入内容" v-model="filters.value">
							...
							<ext-button :label="$t('action.search')" @click="queryForPaginatedList()" icon="el-icon-ali-chazhaobiaodanliebiao" perms="form:view" slot="append" type="primary" />
						</el-input>
					</el-form-item>
					<el-form-item>
						<ext-button :label="$t('action.add')" @click="handleAdd" icon="el-icon-ali-add" perms="form:add" type="primary" />
					</el-form-item>
					<el-form-item>
						<el-button-group>
		...
		<!--表格内容栏-->
		<ext-table :batchDelete="batchDelete" :columns="columns" :count="paginated.attrs.count" :data="paginated.list" :operations="operations" :operationWidth="operationWidth" :pageSize="paginated.attrs.limit" :permsBatchDelete="permsBatchDelete" @queryForPaginatedList="queryForPaginatedList" aliasName="form"></ext-table>

		<!--报表查看界面-->
		<el-dialog :close-on-click-modal="false" :visible.sync="reportDialogVisible" class="dialog-container" title="报表查看" width="40%">
			...
			<div class="dialog-footer" slot="footer">
				<el-button :size="miniSize" @click.native="reportDialogVisible = false" round type="primary">{{$t('action.submit')}}</el-button>
			</div>
		</el-dialog>
		...
		<el-dialog :close-on-click-modal="false" :visible.sync="qrCodeDialogVisible" title="生成二维码" width="40%">
			...
			<div class="dialog-footer" slot="footer">
				<el-button :size="miniSize" @click.native="qrCodeDialogVisible = false" round>{{$t('action.close')}}</el-button>
