<template>
	<div class="page-container">
				...
				<el-form :inline="true" :model="filters" :size="normalSize">
					<el-form-item>
						<el-input placeholder="请输入内容" v-model="filters.value">
							...
							<ext-button :label="$t('action.search')" @click="queryForPaginatedList()" icon="el-icon-ali-chazhaobiaodanliebiao" perms="user:view" slot="append" type="primary" />
						</el-input>
					</el-form-item>
					<el-form-item>
						<ext-button :label="$t('action.add')" @click="handleAdd" icon="el-icon-ali-add" perms="user:add" type="primary" />
					</el-form-item>
					<el-form-item>
						<el-button-group>
		...
		<!--表格内容栏-->
		<ext-table :batchDelete="batchDelete" :columns="columns" :count="paginated.attrs.count" :data="paginated.list" :operations="operations" :operationWidth="operationWidth" :pageSize="paginated.attrs.limit" :permsBatchDelete="permsBatchDelete" @queryForPaginatedList="queryForPaginatedList" aliasName="user"></ext-table>

		<!--新增编辑界面-->
		<el-dialog :close-on-click-modal="false" :title="isCreating?'新增':'编辑'" :visible.sync="editDialogVisible" width="45%">
					...
					<el-col :span="12">
						<el-form-item label="性别" prop="sex">
							<el-select placeholder="请选择" v-model="formData.sex">
								<el-option :key="item.value" :label="item.label" :value="item.value" v-for="item in sexOpts"></el-option>
							</el-select>
						</el-form-item>
					</el-col>
					<el-col :span="12">
					...
					<el-col :span="12">
						<el-form-item label="等级" prop="gradeId">
							<el-select placeholder="请选择" v-model="formData.gradeId">
								<el-option :key="item.id" :label="item.name" :value="item.id" v-for="item in userGrades"></el-option>
							</el-select>
						</el-form-item>
					</el-col>
					<el-col :span="12">
