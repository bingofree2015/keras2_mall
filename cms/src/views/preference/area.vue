<template>
	<div class="page-container">
		...
		<el-row>
			...
			<el-col :span="14" class="top-bar">
				<el-form :inline="true" :size="miniSize">
					<el-form-item>
						<ext-button :label="$t('action.add')" @click="handleAdd" icon="el-icon-ali-add" perms="preference:area:add" type="primary" />
					</el-form-item>
					<el-form-item>
						<el-button-group>
		...
		<!--表格树内容栏-->
		<el-table :data="areaTreeData" element-loading-text="$t('action.loading')" size="mini" stripe style="width: 100%;" v-loading="loading">
			<el-table-column label="ID" minWidth="80" prop="id"></el-table-column>
			<table-tree-column @sendTreeData="getTreeData" label="名称" minWidth="310" prop="name" treeKey="id"></table-tree-column>
			...
			<el-table-column :label="$t('action.operation')" fixed="right" minWidth="200">
				<template slot-scope="scope">
					<ext-button :label="$t('action.edit')" @click="handleEdit(scope.row)" icon="el-icon-ali-bianji" perms="preference:area:edit" />
					<ext-button :label="$t('action.delete')" @click="handleDelete(scope.row)" icon="el-icon-ali-shanchu" perms="preference:area:delete" type="danger" />
				</template>
			</el-table-column>
		</el-table>
				...
				<el-row>
					...
					<el-col :span="12">
						<el-form-item label="上级名称" prop="parentName">
							<popup-tree-input :currentChangeHandle="handleTreeSelectChange" :data="popupTreeData" :nodeKey="''+formData.parentId" :prop="formData.parentName===null||formData.parentName===''?'顶级区域':formData.parentName" :props="popupTreeProps"></popup-tree-input>
						</el-form-item>
					</el-col>
				</el-row>
				<el-form-item label="排序" prop="sort" style="width:80px">
					<el-input-number :min="0" controls-position="right" label="排序" v-model="formData.sort"></el-input-number>
				</el-form-item>
			</el-form>
