<template>
	<el-dialog :close-on-click-modal="false" :modal="false" @open="openDialog" title="多文件上传" v-bind="$attrs" v-on="$listeners" width="60%">
		<el-container style="border: 1px solid #eee">
			<el-aside width="200px">
				<el-table :size="miniSize" :cell-style="{padding:'0px'}" :data="attachGroups" :row-style="{height:'42px'}" @cell-click="cellClick" @cell-dblclick="cellDblClick" highlight-current-row ref="attachGroupTable">
					<el-table-column label="图片分组" prop="name">
						<template slot="header" slot-scope="scope">
							图片分组
							<ext-button :label="$t('action.add')" :size="miniSize" @click="onCreateAttachGroup(scope)" style="float:right;" type="primary" />
						</template>
						<template slot-scope="scope">
							<el-input :size="miniSize" @blur="onSaveAttachGroup(scope.row)" placeholder="请输入内容" style="width:100%" v-if="scope.row.id === editGroupItem.id" v-model="scope.row.name"></el-input>
							<span style="margin-left: 10px" v-else>
								{{ scope.row.name }}
								<el-button :size="miniSize" @click="onDeleteAttachGroup(scope.row)" circle icon="el-icon-delete" style="float:right;" type="danger"></el-button>
							</span>
						</template>
					</el-table-column>
				</el-table>
			...
			<el-main>
				<el-tabs v-model="activeName">
					<el-tab-pane name="local">
						...
						<el-upload :auto-upload="false" :data="extData" :file-list.sync="imageList" :headers="headers" :on-change="changeUpload" action class="image-list" list-type="picture-card" ref="imgUploader">
						...
						<el-pagination :current-page="currPage" :page-size="pageSize" :total="count" @current-change="handleCurrentChange" layout="total, prev, pager, next, jumper" style="float:right;"></el-pagination>
					</el-tab-pane>
					<el-tab-pane name="remote">
						...
						<div style="padding:0px 5px;">
							<el-input placeholder="请粘进图片的Url地址" v-model="downloadImgUrl">
								<el-button @click="onDownloadImage()" icon="el-icon-ali-xiazai" slot="append" v-bind="{ disabled: checkImgUrl(downloadImgUrl) ? false : true }">下载</el-button>
							</el-input>
							<el-image :src="downloadImgUrl" fit="scale-down" style="border-radius: 30px;margin-top:5px;max-height:380px">
								<div class="image-slot" slot="error">
									<i class="el-icon-ali-gaiyemianshanchuhuobucunzai" style="font-size: 180px;color:#F2F6FC"></i>
								</div>
							</el-image>
						</div>
					</el-tab-pane>
			...
			<el-dialog :before-close="closeCropDialog" :visible.sync="cropDialogVisible" append-to-body title="图片剪裁">
				<el-form :inline="true" :size="miniSize">
					<el-container>
						...
						<el-footer>
							<el-form-item>
								<el-button-group>
									<el-button @click="changeScale(1)" icon="el-icon-ali-fangda" round type="primary"></el-button>
									<el-button @click="changeScale(-1)" icon="el-icon-ali-suoxiao" round type="primary"></el-button>
									<el-button @click="rotateLeft" icon="el-icon-ali-left" round type="primary"></el-button>
									<el-button @click="rotateRight" icon="el-icon-ali-right" round type="primary"></el-button>
								</el-button-group>
							</el-form-item>
						</el-footer>
					</el-container>
				...
				<div class="dialog-footer" slot="footer">
					...
					<el-button :loading="loading" :size="normalSize" @click="uploadCropData" round type="primary">确认</el-button>
				</div>
			</el-dialog>
		</el-container>
		<div class="dialog-footer" slot="footer">
			<el-button :size="miniSize" @click.native="$emit('update:visible', false)" round>{{$t('action.cancel')}}</el-button>
			<el-button :disabled="!chosen || (chosen && chosen.id === 0)" :size="miniSize" @click.native="chosedImage" round type="primary">{{$t('action.submit')}}</el-button>
		</div>
	</el-dialog>
</template>
...
.image-list ::v-deep .el-upload--picture-card {
    width: 80px;
    height: 80px;
    line-height: 80px;
    .el-icon-plus {
...
.image-list ::v-deep .el-upload-list__item {
    width: 80px;
    height: 80px;
}
// 截图
