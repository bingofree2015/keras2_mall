<template>
	<div class="page-container">
				...
				<el-form :inline="true" :model="filters" :size="normalSize">
					<el-form-item>
						<el-input placeholder="请输入内容" v-model="filters.value">
							...
							<ext-button :label="$t('action.search')" @click="queryForPaginatedList()" icon="el-icon-ali-chazhaobiaodanliebiao" perms="goods:goods_type:view" slot="append" type="primary" />
						</el-input>
					</el-form-item>
					<el-form-item>
						<ext-button :label="$t('action.add')" @click="handleAdd" icon="el-icon-ali-add" perms="goods:goods_type:add" type="primary" />
					</el-form-item>
					<el-form-item>
						<el-button-group>
		...
		<!--新增编辑界面-->
		<el-dialog :close-on-click-modal="false" :title="isCreating?'新增':'编辑'" :visible.sync="editDialogVisible" width="60%">
						...
						<el-table :data="formData.params" style="width: 100%">
							<el-table-column label="参数名称" minWidth="240" prop="name">
								<template slot-scope="scope">
									<el-autocomplete :fetch-suggestions="queryParams" :size="miniSize" @select="((item)=>handleSelectParam(item,scope.row.idx))" placeholder="请输入内容" v-model="scope.row.name"></el-autocomplete>
								</template>
							</el-table-column>
							...
							<el-table-column align="center" fixed="right" label="操作" minWidth="80">
								<template slot="header">
									<el-button @click.prevent="handleAddParam()" round size="mini" style="float:right" type="primary">添加</el-button>
								</template>
								<template slot-scope="scope">
									<el-button @click="handleDeleteParam(scope.row.idx)" circle icon="el-icon-delete" size="mini" type="danger"></el-button>
								</template>
							</el-table-column>
						</el-table>
					</el-tab-pane>
					<el-tab-pane name="specs">
						...
						<el-table :data="formData.specs" style="width: 100%">
							<el-table-column label="属性名称" minWidth="240" prop="name">
								<template slot-scope="scope">
									<el-autocomplete :fetch-suggestions="querySpecs" :size="miniSize" @select="((item)=>handleSelectSpec(item,scope.row.idx))" placeholder="请输入内容" v-model="scope.row.name"></el-autocomplete>
								</template>
							</el-table-column>
							<el-table-column align="center" label="排序" minWidth="160" prop="sort">
								<template slot-scope="scope">
									<el-input-number :max="99" :min="0" :size="miniSize" controls-position="right" v-model="scope.row.sort"></el-input-number>
								</template>
							</el-table-column>
							...
							<el-table-column align="center" fixed="right" label="操作" minWidth="80">
								<template slot="header">
									<el-button @click.prevent="handleAddSpec()" round size="mini" style="float:right" type="primary">添加</el-button>
								</template>
								<template slot-scope="scope">
									<el-button @click="handleDeleteSpec(scope.row.idx)" circle icon="el-icon-delete" size="mini" type="danger"></el-button>
								</template>
							</el-table-column>
						</el-table>
					</el-tab-pane>
