 ...
 */
function mountRoutes(dynamicRoutes) {
    dynamicRoutes.map(v => {
        if (v.name === "代码生成") {
...
function buildDynamicRoutes(menus) {
    ...
    const menu2Router = (menu, nav = [], routers = []) => {
        ...
        if (_path) {
            // 外链
            _router.path = _path;
            _router.component = () => import(`@/views/layout/iframe`);
            /*
 ...
 * 加载动态菜单和路由
 */
export async function addDynamicMenuAndRoutes(sysUserId) {
    try {
        let _result = await api.menu.getRouteTree({ sysUserId });
        if (_result.succeed === 1 && _result.code === 200) {
            const _routeMenu = _result.data.list;
            // 添加动态路由
            const _dynamicRoutes = buildDynamicRoutes(_routeMenu);
            // 处理静态组件绑定路由
            mountRoutes(_dynamicRoutes);
