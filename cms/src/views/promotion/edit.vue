			...
			<el-col :span="16">
				...
				<el-form :model="formData" :rules="formDataRules" :size="normalSize" label-width="100px" ref="formData">
					...
					<el-row>
						...
						<el-col :span="16">
							<el-form-item label="起止时间" prop="rangeTime">
								<el-date-picker end-placeholder="结束日期" range-separator="至" start-placeholder="开始日期" style="width:360px;" type="datetimerange" v-model="formData.rangeTime"></el-date-picker>
							</el-form-item>
						</el-col>
					</el-row>
					...
					<div style="text-align:right;width:100%">
						<el-select :size="miniSize" @change="changeTargetType" placeholder="请选择" style="float:right" v-model="formData.spTarget.code">
							<el-option :key="idx" :label="item.name" :value="key" v-for="(item,key,idx) in targetTypes"></el-option>
						</el-select>
						<el-col :offset="6" :span="18" class="target-container" v-if="targetTypes[formData.spTarget.code]">
							...
							<div v-else-if="formData.spTarget.code === 'GOODS_IDS'">
								...
								<pick-goods :selectionType="0" @chosedGoods="(chosed)=>{chosedGoods(chosed,formData.spTarget.pattern)}"></pick-goods>
								<el-table :data="formData.spTarget.pattern.goods" :size="miniSize" stripe style="width: 100%">
									...
									<el-table-column fixed="right" label="操作" min-width="88" prop="operation">
										<template slot-scope="scope">
											<ext-button :label="$t('action.delete')" :size="miniSize" @click="deleteGoods(scope.$index)" icon="el-icon-ali-shanchu" type="danger" />
										</template>
									</el-table-column>
								</el-table>
								<el-form-item label="商品数量">
									<el-input-number :max="10" :min="1" controls-position="right" v-model="formData.spTarget.pattern.discount"></el-input-number>
									<span class="tip-info">
							...
							<div v-else-if="formData.spTarget.code === 'GOODS_CATS'">
								...
								<el-form-item label="商品分类" prop="id">
									<el-cascader :options="goodsCatList" :props="cascaderProps" :show-all-levels="false" v-model="formData.spTarget.pattern.id"></el-cascader>
								</el-form-item>
								<el-form-item label="商品数量">
									<el-input-number :max="10" :min="1" controls-position="right" v-model="formData.spTarget.pattern.discount"></el-input-number>
									<span class="tip-info">
							...
							<div v-else-if="formData.spTarget.code === 'ORDER_FULL'">
								...
								<el-form-item label="满多少">
									<el-input-number :max="10" :min="1" controls-position="right" v-model="formData.spTarget.pattern.discount"></el-input-number>订单金额满多少的时候，优惠
								</el-form-item>
							</div>
							<div v-else-if="formData.spTarget.code === 'USER_GRADE'">
								...
								<el-form-item label="请选择" prop="id">
									<el-checkbox-group @change="chosedUerGrade" v-model="formData.spTarget.pattern.id">
										<el-checkbox :key="idx" :label="item.id" v-for="(item, idx) in userGradeList">{{item.name}}</el-checkbox>
									</el-checkbox-group>
								</el-form-item>
							</div>
							<el-form-item>
								<el-button :size="miniSize" @click.native="addTarget" round style="float:right" type="primary">添加商品筛选条件</el-button>
							</el-form-item>
						</el-col>
					</div>
					...
					<el-table :data="formData.spTargets" :size="miniSize" stripe style="width: 100%">
						...
						<el-table-column fixed="right" label="操作" min-width="88" prop="operation">
							<template slot-scope="scope">
								<ext-button :label="$t('action.delete')" :size="miniSize" @click="deleteTarget(scope.$index)" icon="el-icon-ali-shanchu" type="danger" />
							</template>
						</el-table-column>
					</el-table>
					...
					<div style="text-align:right;width:100%">
						<el-select :size="miniSize" @change="changeRuleType" placeholder="请选择" v-model="formData.spRule.code">
							<el-option :key="idx" :label="item.name" :value="key" v-for="(item,key,idx) in ruleTypes"></el-option>
						</el-select>
						<el-col :offset="6" :span="18" class="rules-container" v-if="ruleTypes[formData.spRule.code]">
							<div v-if="formData.spRule.code === 'GOODS_REDUCE'">
								...
								<el-form-item label="金额">
									<el-input-number :max="10" :min="1" controls-position="right" v-model="formData.spRule.pattern.money"></el-input-number>
									<span class="tip-info">
							...
							<div v-else-if="formData.spRule.code === 'GOODS_DISCOUNT'">
								...
								<el-form-item label="折扣">
									<el-input-number :max="10" :min="1" controls-position="right" v-model="formData.spRule.pattern.discount"></el-input-number>
									<span class="tip-info">
							...
							<div v-else-if="formData.spRule.code === 'GOODS_HALF_PRICE'">
								...
								<el-form-item label="第几件">
									<el-input-number :max="10" :min="1" controls-position="right" v-model="formData.spRule.pattern.num"></el-input-number>
									<span class="tip-info">
							...
							<el-form-item>
								<el-button :size="miniSize" @click.native="addRule" round style="float:right" type="primary">添加规则</el-button>
							</el-form-item>
						</el-col>
					</div>
					...
					<el-table :data="formData.spRules" :size="miniSize" stripe style="width: 100%">
						...
						<el-table-column fixed="right" label="操作" min-width="88" prop="operation">
							<template slot-scope="scope">
								<ext-button :label="$t('action.delete')" :size="miniSize" @click="deleteRule(scope.$index)" icon="el-icon-ali-shanchu" type="danger" />
							</template>
						</el-table-column>
					</el-table>
				</el-form>
