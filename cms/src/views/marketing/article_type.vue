<template>
	<div class="page-container">
		...
		<el-row>
			...
			<el-col :span="14" class="top-bar">
				<el-form :inline="true" :size="miniSize">
					<el-form-item>
						<ext-button :label="$t('action.add')" @click="handleAdd" icon="el-icon-ali-add" perms="marketing:article_type:add" type="primary" />
					</el-form-item>
					<el-form-item>
						<el-button-group>
		...
		<!--表格树内容栏-->
		<el-table :data="articleTypeTreeData" element-loading-text="$t('action.loading')" size="mini" stripe style="width: 100%;" v-loading="loading">
			<el-table-column align="center" label="ID" minWidth="60" prop="id"></el-table-column>
			<table-tree-column @sendTreeData="getTreeData" label="名称" minWidth="280" prop="typeName" treeKey="id"></table-tree-column>
			...
			<el-table-column :label="$t('action.operation')" fixed="right" minWidth="200">
				<template slot-scope="scope">
					<ext-button :label="$t('action.edit')" @click="handleEdit(scope.row)" icon="el-icon-ali-bianji" perms="marketing:article_type:edit" />
					<ext-button :label="$t('action.delete')" @click="handleDelete(scope.row)" icon="el-icon-ali-shanchu" perms="marketing:article_type:delete" type="danger" />
				</template>
			</el-table-column>
		</el-table>
				...
				<el-row>
					<el-col :span="16">
						<el-form-item label="上级分类" prop="parentName">
							<popup-tree-input :currentChangeHandle="handleTreeSelectChange" :data="popupTreeData" :nodeKey="''+formData.pid" :prop="formData.parentName===null||formData.parentName===''?'顶级分类':formData.parentName" :props="popupTreeProps"></popup-tree-input>
						</el-form-item>
					</el-col>
				</el-row>
				<el-row>
