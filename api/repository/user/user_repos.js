import { User, UserGrade } from '../../models';
...
class UserRepos {
  ...
  async create (user) {
    ...
    try {
      ...
      if (user.mobile) {
        let _user = await User.findOne({ where: { mobile: user.mobile }, raw: true });
        if (_user) {
          _result = { succeed: 0, code: 101, description: '手机号重复' };
        } else {
        ...
        _result = { succeed: 0, code: 100, description: `参数错误 -> user:${JSON.stringify(user)}` };
      }
    } catch (err) {
      logger.error(err);
  ...
  async update (id, user) {
    ...
    try {
      ...
      if (id) {
        ...
        if (_username) {
          let _user = await User.findOne({ where: { id, username: _username }, raw: true });
          if (_user) {
            ...
            _user = await User.findOne({
              where: { username: _username, id: { $ne: id } },
              raw: true,
            });
            if (_user) {
              _result = { succeed: 0, code: 101, description: `名称 [${_username}] 重复` };
            } else {
              const _ret = await User.update(user, { where: { id } });
              const _affectedCount = _ret[0];
              if (_affectedCount == 0) {
    ...
    try {
      if ((offset == 0 || offset) && limit) {
        ...
        _result = {
          ...
          code: 200,
          description: '成功',
          data: { list: _datas, count: _users.count },
        };
      } else {
...
export default UserRepos.getInstance();
