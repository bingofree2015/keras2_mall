<template>
	<el-dialog :close-on-click-modal="false" :modal="false" class="video-dialog" title="视频上传" v-bind="$attrs" v-on="$listeners" width="40%">
		<el-container>
			<el-main>
				<el-row>
					<el-col :span="24" class="toolbar">
						<el-button :size="miniSize" @click="cancelUpload" round type="primary" v-if="uploadProgressState === 1 || uploadProgressState === 3">取消</el-button>
						<el-button :loading="uploadLoading" :size="miniSize" @click="uploadVideo" round type="primary" v-if="uploadProgressState === 1">上传</el-button>
					</el-col>
				</el-row>
				<el-row>
					<el-col :span="24">
						<el-upload :auto-upload="false" :on-change="changeUpload" :show-file-list="false" action drag v-if="uploadProgressState === 0">
							<i class="el-icon-upload"></i>
							<div class="el-upload__text">点击上传</div>
							<div class="el-upload__tip">支持绝大多数视频格式，单个视频最大支持500MB</div>
						</el-upload>

						<video class="video-container" controls="controls" ref="videoPlayer" v-else-if="uploadProgressState > 0">您的浏览器不支持视频播放</video>
					</el-col>
				</el-row>
				<el-row>
					<el-col :span="24">
						<el-progress v-bind:percentage="videoUploadPercent" v-if="videoUploadPercent > 0 && videoUploadPercent < 100"></el-progress>
					</el-col>
				</el-row>
			</el-main>
		</el-container>
		<div class="dialog-footer" slot="footer">
			<el-button :size="miniSize" @click.native="$emit('update:visible', false)" round>{{$t('action.close')}}</el-button>
			<el-button :disabled="!videoUrl" :size="miniSize" @click.native="chosedVideo" round type="primary">{{$t('action.submit')}}</el-button>
		</div>
	</el-dialog>
</template>
