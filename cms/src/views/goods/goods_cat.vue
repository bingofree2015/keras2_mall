<template>
	<div class="page-container">
		...
		<el-row>
			...
			<el-col :span="14" class="top-bar">
				<el-form :inline="true" :size="miniSize">
					<el-form-item>
						<ext-button :label="$t('action.add')" @click="handleAdd" icon="el-icon-ali-add" perms="goods:goods_cat:add" type="primary" />
					</el-form-item>
					<el-form-item>
						<el-button-group>
		...
		<!--表格树内容栏-->
		<el-table :data="goodsCatTreeData" element-loading-text="$t('action.loading')" size="mini" stripe style="width: 100%;" v-loading="loading">
			<el-table-column align="center" label="ID" minWidth="60" prop="id"></el-table-column>
			<table-tree-column @sendTreeData="getTreeData" label="名称" minWidth="280" prop="name" treeKey="id"></table-tree-column>
			<el-table-column align="center" label="图标" minWidth="80">
				<template slot-scope="scope">
					<el-avatar :size="30" :src="env.getImgUrl(scope.row.attachment.path)" v-if="scope.row.attachment && scope.row.attachment.path"></el-avatar>
				</template>
			</el-table-column>
			...
			<el-table-column :label="$t('action.operation')" fixed="right" minWidth="200">
				<template slot-scope="scope">
					<ext-button :label="$t('action.edit')" @click="handleEdit(scope.row)" icon="el-icon-ali-bianji" perms="goods:goods_cat:edit" />
					<ext-button :label="$t('action.delete')" @click="handleDelete(scope.row)" icon="el-icon-ali-shanchu" perms="goods:goods_cat:delete" type="danger" />
				</template>
			</el-table-column>
		</el-table>

		<!-- 新增修改界面 -->
		<el-dialog :close-on-click-modal="false" :title="!formData.id ? '新增' : '修改'" :visible.sync="dialogVisible" width="40%">
			<el-form :model="formData" :rules="dataRule" :size="normalSize" @keyup.enter.native="submitForm()" label-width="80px" ref="formData">
				<el-row>
					<el-col :span="16">
						...
						<el-form-item label="上级分类" prop="parentName">
							<popup-tree-input :currentChangeHandle="handleTreeSelectChange" :data="popupTreeData" :nodeKey="''+formData.parentId" :prop="formData.parentName===null||formData.parentName===''?'顶级分类':formData.parentName" :props="popupTreeProps"></popup-tree-input>
						</el-form-item>
					</el-col>
					<el-col :span="8">
						<el-form-item label="分类图标" prop="attachment">
							<change-image-icon :imgUrl="formData.attachment ? formData.attachment.path : ''" @chosedImageIcon="chosedLogo"></change-image-icon>
						</el-form-item>
					</el-col>
				</el-row>
				<el-row>
					...
					<el-col :span="8">
						<el-form-item label="排序编号" prop="sort">
							<el-input-number :min="0" controls-position="right" label="排序编号" style="width:100px" v-model="formData.sort"></el-input-number>
						</el-form-item>
					</el-col>
				</el-row>
			</el-form>
			<span class="dialog-footer" slot="footer">
				...
				<el-button :size="miniSize" @click="submitForm()" round type="primary">{{$t('action.submit')}}</el-button>
			</span>
		</el-dialog>
	</div>
</template>
