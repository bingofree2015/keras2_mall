<template>
    <div>
        <ext-button :size="miniSize" @click="handlePickNotices()" icon="el-icon-ali-Newxuanzeshangpinxuanzhong" label="选择公告" type="primary" />
        <el-dialog :modal-append-to-body="false" :close-on-click-modal="false" :size="miniSize" :visible.sync="dialogVisible" title="公告选择窗" width="50%">
            <el-table :data="paginated.list" :size="miniSize" @selection-change="multipleChoiceChange" element-loading-text="$t('action.loading')" stripe v-loading="loading">
                ...
                <el-table-column align="center" label="选择" minWidth="40" v-else-if="selectionType === 0">
                    <template slot-scope="scope">
                        <el-radio @change.native="(selectionType==0) && choiceChange(scope.$index,scope.row)" :label="scope.$index" v-model="radio">&nbsp;</el-radio>
                    </template>
                </el-table-column>

                <el-table-column label="标题" min-width="310" property="title" show-overflow-tooltip></el-table-column>
                <el-table-column label="时间" min-width="120" property="createdAt" formatter="env.formatDateTime"></el-table-column>
            </el-table>
            <div style="padding:10px;">
                <el-pagination :current-page="paginated.attrs.currPage" :page-size="paginated.attrs.limit" :total="paginated.attrs.count" @current-change="handleCurrentChange" layout="total, prev, pager, next, jumper" style="float:right;"></el-pagination>
            </div>
