<template>
	<div class="page-container">
				...
				<el-form :inline="true" :model="filters" :size="normalSize">
					<el-form-item>
						<el-input placeholder="请输入内容" v-model="filters.value">
							...
							<ext-button :label="$t('action.search')" @click="queryForPaginatedList()" icon="el-icon-ali-chazhaobiaodanliebiao" perms="form:form_submit:view" slot="append" type="primary" />
						</el-input>
					</el-form-item>
					<el-form-item>
						<el-button-group>
		...
		<!--表格内容栏-->
		<ext-table :columns="columns" :count="paginated.attrs.count" :data="paginated.list" :operations="operations" :operationWidth="operationWidth" :pageSize="paginated.attrs.limit" @queryForPaginatedList="queryForPaginatedList" aliasName="form_submit"></ext-table>

		<el-dialog :close-on-click-modal="false" :visible.sync="viewDialogVisible" class="dialog-container" title="表单提交明细" width="50%">
		...
		<el-dialog :close-on-click-modal="false" :visible.sync="feedbackDialogVisible" title="反馈表单" width="40%">
			<el-form :model="formData" :rules="formDataRules" :size="miniSize" label-width="80px" ref="formData">
				<el-form-item label="反馈" prop="feedback">
					<el-input :autosize="{ minRows: 2, maxRows: 4}" placeholder="请输入内容" type="textarea" v-model="formData.feedback"></el-input>
				</el-form-item>
			</el-form>
			<div class="dialog-footer" slot="footer">
				<el-button :size="miniSize" @click.native="feedbackDialogVisible = false" round type="danger">{{$t('action.cancel')}}</el-button>
				<el-button :size="miniSize" @click.native="submitForm" @load="editLoading" round type="primary">{{$t('action.submit')}}</el-button>
			</div>
		</el-dialog>
	</div>
</template>
