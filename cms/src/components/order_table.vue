<template>
    <el-container>
        <!--表格栏-->
        <el-main>
            <el-table :align="align" :border="border" :data="orderData" :element-loading-text="$t('action.loading')" :highlight-current-row="highlightCurrentRow" :max-height="maxHeight" :size="miniSize" :stripe="stripe" @current-change="handleCurrentRowChange" @selection-change="selectionChange" style="width:100%;" v-loading="loading">
                ...
                <el-table-column :formatter="(row,column)=>{return env.formatDateTime(row[column.property])}" label="下单时间" minWidth="150" prop="createdAt"></el-table-column>
                <el-table-column label="订单状态" minWidth="90" prop="progress"></el-table-column>
                <el-table-column label="售后状态" minWidth="180" prop="afterSaleState" show-overflow-tooltip></el-table-column>
                ...
                <el-table-column :label="$t('action.operation')" :minWidth="operationWidth" fixed="right">
                    <template slot-scope="scope">
                        <template v-for="(item,idx) in scope.row['operatings']">
                            <ext-button :icon="item.icon" :key="idx" :label="$t(item.action)" :perms="item.perms" @click="handleAction(scope.row,item.action)" />
                        </template>
                    </template>
                </el-table-column>
            </el-table>
        ...
        <el-footer style="height:42px">
            <ext-button :disabled="this.selections.length===0" :label="$t('action.batchDelete')" :perms="permsBatchDelete" :size="miniSize" @click="handleBatchDelete()" style="float:left;" type="danger" v-if="batchDelete" />
            <el-pagination :current-page="attrs.currPage" :page-size="pageSize" :total="count" @current-change="handleCurrentChange" layout="total, prev, pager, next, jumper" style="float:right;"></el-pagination>
        </el-footer>
    </el-container>
</template>
