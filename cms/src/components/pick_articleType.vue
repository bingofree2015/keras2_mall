<template>
    <div>
        <ext-button :size="miniSize" @click="handlePickArticleType()" icon="el-icon-ali-Newxuanzeshangpinxuanzhong" label="选择文章分类" type="primary" />
        <el-dialog :close-on-click-modal="false" :modal-append-to-body="false" :size="miniSize" :visible.sync="dialogVisible" title="文章分类选择窗" width="40%">
            <el-table :data="articleTypeData" :size="miniSize" @selection-change="multipleChoiceChange" element-loading-text="$t('action.loading')" stripe v-loading="loading">
                ...
                <el-table-column align="center" label="选择" minWidth="80" v-else-if="selectionType === 0">
                    <template slot-scope="scope">
                        <el-radio :label="scope.$index" @change.native="choiceChange(scope.$index,scope.row)" v-model="radio">&nbsp;</el-radio>
                    </template>
                </el-table-column>
                <el-table-column label="分类名称" min-width="210" property="typeName" show-overflow-tooltip></el-table-column>
                <el-table-column label="创建时间" min-width="130" :formatter="env.formatDateTime" property="createdAt"></el-table-column>
            </el-table>
