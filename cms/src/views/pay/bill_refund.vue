<template>
	<div class="page-container">
				...
				<el-form :inline="true" :model="filters" :size="normalSize">
					<el-form-item>
						<el-input placeholder="请输入内容" v-model="filters.value">
							...
							<ext-button :label="$t('action.search')" @click="queryForPaginatedList()" icon="el-icon-ali-chazhaobiaodanliebiao" perms="pay:bill_refund:view" slot="append" type="primary" />
						</el-input>
					</el-form-item>
					<el-form-item>
						<el-button-group>
		...
		<!--表格内容栏-->
		<ext-table :columns="columns" :count="paginated.attrs.count" :data="paginated.list" :operations="operations" :operationWidth="operationWidth" :pageSize="paginated.attrs.limit" @queryForPaginatedList="queryForPaginatedList" aliasName="bill_refund"></ext-table>
		...
		<el-dialog :close-on-click-modal="false" :visible.sync="checkDialogVisible" title="退款单审核" width="40%">
					...
					<el-col :span="12">
						<el-form-item label="退款方式:" prop="paymentCode">
							<el-select placeholder="请选择" v-model="formData.paymentCode">
								<el-option :key="item.code" :label="item.name" :value="item.code" v-for="item in paymentOpts"></el-option>
							</el-select>
						</el-form-item>
					</el-col>
					<el-col :span="12">
			...
			<div class="dialog-footer" slot="footer">
				<el-button :size="miniSize" @click.native="checkDialogVisible = false" round>{{$t('action.cancel')}}</el-button>
				<el-button :loading="loading" :size="miniSize" @click.native="submitForm" round type="primary">{{$t('action.submit')}}</el-button>
			</div>
		</el-dialog>

		<!--退款单查看界面-->
		<el-dialog :close-on-click-modal="false" :visible.sync="viewDialogVisible" class="edit-dialog" title="退款单查看" width="40%">
			<el-form :model="formData" :size="miniSize" class="dialog-container" label-width="80px" ref="formData">
				...
				<el-row>
					...
					<el-col :span="12">
						<el-form-item label="状态:" prop="state">{{env.columnFormatter(formData.state,'bill_refund','state')}}</el-form-item>
					</el-col>
				</el-row>
				<el-row>
					<el-col :span="12">
				...
				<el-row>
					<el-col :span="12">
						<el-form-item label="单据类型:" prop="type">{{env.columnFormatter(formData.type,'bill_refund','type')}}</el-form-item>
					</el-col>
					<el-col :span="12">
						<el-form-item label="订单单号:" prop="sourceId">{{formData.sourceId}}</el-form-item>
					</el-col>
			...
			<div class="dialog-footer" slot="footer">
				<el-button :size="miniSize" @click.native="viewDialogVisible = false" round type="primary">{{$t('action.comfirm')}}</el-button>
			</div>
		</el-dialog>
	</div>
</template>
