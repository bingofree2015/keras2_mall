    ...
    try {
      ...
      if (Number.isInteger(article.typeId) && article.title) {
        let _article = await Article.findOne({
          where: { typeId: article.typeId, title: article.title },
          raw: true,
        });
        if (_article) {
      ...
      logger.error(err);
      _result = { succeed: 0, code: 500, description: err.article || err.stack || '系统错误' };
    }
    ...
    try {
      ...
      if (id) {
        ...
        if (Number.isInteger(_typeId) && _title) {
          let _article = await Article.findOne({
            where: { id, typeId: _typeId, title: _title },
            raw: true,
          });
          if (_article) {
            ...
            _article = await Article.findOne({
              where: { typeId: _typeId, title: _title, id: { $ne: id } },
              raw: true,
            });
            if (_article) {
              _result = { succeed: 0, code: 101, description: '记录重复' };
    ...
    try {
      if ((offset == 0 || offset) && limit) {
        ...
        _result = {
          ...
          code: 200,
          description: '成功',
          data: { list: _datas, count: _articles.count },
        };
      } else {
