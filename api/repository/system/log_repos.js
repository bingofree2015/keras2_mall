...
class LogRepos {
    ...
    async create (log) {
        ...
        try {
            ...
            if (log.userId && log.title) {
                let _log = await Log.findOne({ where: { userId: log.userId, title: log.title }, raw: true });
                if (_log) {
                ...
                _result = { succeed: 0, code: 100, description: `参数错误 -> log:${JSON.stringify(log)}` };
            }
        } catch (err) {
            logger.error(err);
    ...
    async update (id, log) {
        ...
        try {
            ...
            if (id) {
                ...
                if (_userId && _title) {
                    let _log = await Log.findOne({ where: { id, userId: _userId, title: _title }, raw: true });
                    if (_log) {
    ...
    async get (id) {
        ...
        try {
            const _log = await Log.findOne({ where: { id }, raw: true });
            if (_log) {
                _result = {
                    succeed: 1,
                    code: 200,
    ...
    async list (searchKey, offset, limit) {
        ...
        try {
            if ((offset == 0 || offset) && limit) {
                ...
                const _logs = await Log.findAll({ where: _where, raw: true });
